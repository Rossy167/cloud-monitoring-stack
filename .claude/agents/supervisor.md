...
3. **Correctness** — run read-only checks only, but actually run them rather than reasoning about them from the diff alone, whenever the tool is available:
   ...
   - `validate`/`fmt`/`plan` passing is NOT evidence that a resource type or its arguments are correct — they only check syntax and schema shape, not whether the cloud provider's API will accept a given role/argument/resource-type combination. This project has hit this exact failure live: a resource passed every static check and was still rejected by GCP on real `apply` because the IAM role needed a different, more specific binding resource type. If a diff introduces a resource type genuinely new to this repo — especially IAM/permission-binding resources, which have many GCP-specific "this role only works on that exact binding resource" gotchas — don't just trust the coder's claim or your own recall: run `terraform providers schema -json` against this repo's actual installed provider (`terraform init -backend=false` first if needed) and read the real schema for that resource type yourself. This is ground truth for the exact pinned provider version, needs no network access, and is more reliable than web docs that can be stale relative to the version in use here.
