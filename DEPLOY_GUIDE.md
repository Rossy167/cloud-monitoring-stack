...
Everything in this repo has been reviewed and merged to `main`. Nothing has been applied to a real GCP project yet, and that part is yours, deliberately: no agent in this workflow is permitted to run `terraform apply`, `terraform plan`, or any `gcloud` command. This is the exact sequence to run it yourself.
...
- SSH in and run `sudo fail2ban-client status sshd`; it should report the jail as active (this was a genuine bug fixed in this round of work; confirm it took effect on your real boot).
...
This does not disable the Compute Engine API on your project (`disable_on_destroy = false`), since that's a project-level setting other things may depend on.
