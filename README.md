...
2. Run `sudo cloud-init status`; it should say `status: done`. If it says `running`, wait longer; if `error`, continue below.
...
- **Tailscale** (optional, `tailscale_auth_key`): if set, the VM joins your tailnet on first boot. SSH in once via one of the two paths above, run `tailscale ip -4` to get its stable tailnet address, then SSH to that address going forward, since it doesn't change even if the VM's public IP or your home IP does. Traffic arrives over the `tailscale0` overlay interface, not through GCP's firewall, so no firewall rule was added for it.
