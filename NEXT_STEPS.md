...
If you're wondering why a previous run left this repo in a mess, here's a quick record:
...
- **Root cause of the concurrency mess**: a prior session spawned multiple coder agents in parallel, each in its own isolated git worktree. Several got stuck mid-task on the approval wall above and were left as locked worktrees with uncommitted work; others finished but were never actually merged into `main` (three "already merged"-looking commits in `git log --all` were actually just sitting on unmerged branches; it's worth double-checking with `git log main..<branch>`, not just `git log --all`, before assuming something landed).
...
- Whether `ssh_source_ranges` should support more than a static IP/32 (e.g. a note on what to do if your home IP changes). Currently it's just documented as "find your IP once," no dynamic-DNS handling.
